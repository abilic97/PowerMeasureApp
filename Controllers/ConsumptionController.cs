...
namespace PowerMeasure.Controllers
{
    ...
    public class ConsumptionController : Controller
    {
        ...
        public async Task<Measurements> getUserTotalnergyValuePerDay(int userId, [FromQuery(Name = "date")] string date)
        {
            DateTime mydate;
            mydate = DateTime.Parse(date);
            return  _consumptionService.getTotalConsumedEnergyValuePerDay(userId, mydate);
        }
        ...
        public EnergyConsumed getUserLastEnergyValues(int userId, [FromQuery(Name = "date")] string date)
        {
             DateTime mydate;
            mydate = DateTime.Parse(date);
            return _consumptionService.getLastEnergyValueDay(userId, mydate);
        }
        [Authorize]
        ...
        public async Task<double> getdailycost(int userId, [FromQuery(Name = "date")] string date)
        {
            ...
            return await _consumptionService.getTotalCostPerDay(userId, mydate);
        }
        ...
        public async Task<IEnumerable<EnergyConsumed>> getAllIndividualEnergy(int userId, [FromQuery(Name = "date")] string date)
        {
            DateTime mydate;
            mydate = DateTime.Parse(date);
            return await _consumptionService.getAllConsumedEnergyValuePerDayIndividual(userId, mydate);
        }
        ...
        public EnergyConsumed getLastIndividualValue(int userId, [FromQuery(Name = "date")] string date)
        {
            DateTime mydate;
            mydate = DateTime.Parse(date);
            return  _consumptionService.getLastEnergyValueDayIndividual(userId, mydate);
        }
    }
}
